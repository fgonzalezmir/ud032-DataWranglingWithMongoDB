...
DATAFILE = r'C:\Users\Usuario\Desktop\arachnid.csv'
...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            dic = {}
            clasif={}
            label = re.compile('\s*\-*\(+\S+$').sub("",line['rdf-schema#label']).strip(' ')
            for k in line:
                if k in fields:
                    new_key = fields[k]
                    if new_key == 'label':  
                    ...
                    if new_key == 'name':
                        if line[k] == "NULL" or re.match('\W',line[k]):
                            dic[new_key] = label
                        else:
                            dic[new_key] = line[k].strip(' ')
                        continue
                    ...
                    if new_key == 'synonym':
                        if line[k] == "NULL":
                            ...
                            aux= line[k].replace('{','').replace('}','')
                            l = aux.split('|')
                            dic[new_key]=[]
                            for a in l:
                                dic[new_key].append(re.compile('\*+\s*').sub("",a).strip(' '))
                        continue
                    
                    if new_key == 'description' or new_key == 'uri':
                        if line[k] == "NULL":
                            dic[new_key] = None
                        else:
                            dic[new_key] = line[k].strip(' ')
...
#                        if new_key == 'class':
#                            l = line[k].lstrip('{').rstrip('}').split('|')
#                            clasif[new_key] = l
#                        else:
                        clasif[new_key] = line[k].strip(' ')
            
            dic["classification"]=clasif            
            data.append(dic)
